...
/*Post: simplifica al màxim l'arbre que li estem passant */
{
    ...
    return expressio(token("maipotserbuit"));
}
...
{
    expressio er = expressio(token("other"));
    if (not tree.es_buit()){

        expressio iz = expressio("tamal") ;
        if (not tree.fe().es_buit())
            iz = expandeix_recursiu(tree.fe(), t, e);

        expressio dr = expressio("tamal") ;
        if (not tree.fd().es_buit())
            dr = expandeix_recursiu(tree.fd(), t, e);

        if (er.arrel() == t){
            er = e;
        } else { 
            if (tree.fe().es_buit() and tree.fd().es_buit()){
                er = expressio(tree.arrel(),iz,dr); 
            }
        }
...
// Post: El p.i. conté l'expressió llegida del canal is.
{
    ...
    while (is>>s and s != "->"){
        token x = token(s);
        if (x.to_string()=="(") {
            operadors.push(x);   //si es un ) desapilarem els operadors fins a trobar un (
        } else if (x.to_string()==")"){ 
            while (operadors.top().to_string() != "("){
                arbreBin<token>tree;
                if (operadors.top().es_operador_binari()){
                    arbreBin<token> primer = operands.top();
                    operands.pop();
                    arbreBin<token> segon = operands.top();
                    operands.pop();
                    tree=arbreBin<token>(operadors.top(),segon, primer);
                } else if (operadors.top().es_operador_unari()){
                    arbreBin<token> primer = operands.top();
                    operands.pop();
                    tree=arbreBin<token>(operadors.top(),primer,arbreBin<token>());
                }
                operands.push(tree);
                operadors.pop();
            }
            operadors.pop();
        } else if (x.es_enter() or x.es_boolea() or x.es_variable()) {
            operands.push(arbreBin<token>(x,arbreBin<token>(),arbreBin<token>()));
        }
        else if (x.es_operador_binari() or x.es_operador_unari()){
            if (not operadors.empty()){
                token pila_operador = operadors.top();
                if (pila_operador != "(" and (x.prioritat().first < pila_operador.prioritat().first or (x.prioritat().first ==  pila_operador.prioritat().first and x.prioritat().second == true))){
                    operadors.pop();
                    arbreBin<token>tree;
                    if (pila_operador.es_operador_binari()){
                        arbreBin<token> primer = operands.top();
                        operands.pop();
                        arbreBin<token> segon = operands.top();
                        operands.pop();
                        tree=arbreBin<token>(x,segon,primer);
                    } else if (pila_operador.es_operador_unari()){
                        arbreBin<token> primer = operands.top();
                        operands.pop();
                        tree=arbreBin<token>(x, primer, arbreBin<token>());
                    }
                    operands.push(tree);
                }
            }
    ...
    while (not operadors.empty()){ 
        token alto = operadors.top();
        operadors.pop(); 
        if (alto.to_string() != "(" or alto.to_string() != ")"){ 
            arbreBin<token>tree;
            if (alto.es_operador_binari()){
                arbreBin<token> primer = operands.top();
                operands.pop();
                arbreBin<token> segon = operands.top();
                operands.pop();
                tree=arbreBin<token>(alto,segon,primer);
            } else if (alto.es_operador_unari()){
                arbreBin<token> primer = operands.top();
                operands.pop();
                tree=arbreBin<token>(alto,primer, arbreBin<token>());
            }
            operands.push(tree);
        }
    }
    ...
    if (e1.es_operand() and e2.es_operand()){
        ...
        if (op.to_string() == "=="){
            if (e1.arrel().es_enter() and e2.arrel().es_enter()){
                ...
                return expressio(token(e1.arrel().to_string()==e2.arrel().to_string()));
            }
